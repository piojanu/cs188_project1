...
def parseHeuristic(heuristicText):
    ...
    def graphHeuristic(state, problem=None):
        if state in heuristic:
            ...
            pp.pprint(heuristic)
            raise Exception(
                "Graph heuristic called with invalid state: " +
                str(state))
...
class GraphSearchTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        gold_solution = [
            str.split(
                solutionDict['solution']), str.split(
                solutionDict['rev_solution'])]
        gold_expanded_states = [
            str.split(
                solutionDict['expanded_states']), str.split(
                solutionDict['rev_expanded_states'])]
        ...
        if solution in gold_solution and (
            ...
            grades.addMessage(
                '\tstudent expanded_states:\t%s' %
                expanded_states)
            grades.addMessage('')
            grades.addMessage('\tcorrect solution:\t\t%s' % gold_solution[0])
            grades.addMessage(
                '\tcorrect expanded_states:\t%s' %
                gold_expanded_states[0])
            grades.addMessage(
                '\tcorrect rev_solution:\t\t%s' %
                gold_solution[1])
            grades.addMessage(
                '\tcorrect rev_expanded_states:\t%s' %
...
class PacmanSearchTest(testClasses.TestCase):
    ...
    def getSolInfo(self, search, searchAgents):
        ...
        heuristic = getattr(
            searchAgents,
            self.heuristicName) if self.heuristicName is not None else None
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        gold_solution = [
            str.split(
                solutionDict['solution']), str.split(
                solutionDict['rev_solution'])]
        gold_expanded = max(int(solutionDict['expanded_nodes']), int(
            solutionDict['rev_expanded_nodes']))
        ...
        if solution not in gold_solution:
            ...
            grades.addMessage('')
            grades.addMessage(
                '\tcorrect solution length: %s' % len(
                    gold_solution[0]))
            grades.addMessage(
                '\tcorrect (reversed) solution length: %s' % len(
                    gold_solution[1]))
            grades.addMessage(
                '\tcorrect solution:\n%s' %
                wrap_solution(
...
class CornerHeuristicPacman(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if points >= len(thresholds):
            grades.addMessage(
                'PASS: Heuristic resulted in expansion of %d nodes' %
                expanded)
        else:
            grades.addMessage(
                'FAIL: Heuristic resulted in expansion of %d nodes' %
                expanded)
        return True
